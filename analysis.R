...
test_set <- readData("test.csv")
#test_set <- test_set[!names(test_set) %in% c("City","City.Group")]
train_set <- readData("train.csv")
...
write.table(res_out, file = "pred_log_best.csv", quote = F, sep = ",",row.names = F)
